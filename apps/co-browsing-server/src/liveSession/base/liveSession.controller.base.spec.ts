...
const CREATE_INPUT = {
  ...
  endTime: new Date(),
  id: "exampleId",
  liveSessionId: "exampleLiveSessionId",
  startTime: new Date(),
  updatedAt: new Date(),
...
const service = {
  ...
  liveSessions: () => FIND_MANY_RESULT,
  liveSession: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("LiveSession", () => {
  ...
  test("GET /liveSessions/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/liveSessions"}/${nonExistingId}`)
  ...
  test("GET /liveSessions/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/liveSessions"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /liveSessions existing resource", async () => {
