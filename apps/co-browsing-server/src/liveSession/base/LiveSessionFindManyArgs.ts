/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { LiveSessionWhereInput } from "./LiveSessionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { LiveSessionOrderByInput } from "./LiveSessionOrderByInput";

@ArgsType()
class LiveSessionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => LiveSessionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => LiveSessionWhereInput, { nullable: true })
  @Type(() => LiveSessionWhereInput)
  where?: LiveSessionWhereInput;

  @ApiProperty({
    required: false,
    type: [LiveSessionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [LiveSessionOrderByInput], { nullable: true })
  @Type(() => LiveSessionOrderByInput)
  orderBy?: Array<LiveSessionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { LiveSessionFindManyArgs as LiveSessionFindManyArgs };
